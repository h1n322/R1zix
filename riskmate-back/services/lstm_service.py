...
if __name__ == "__main__":
    ...
    # Підключаємо шлях до наших модулів
    sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from data.ml_processor import prepare_data_for_lstm
 # 🌟 ТОП-20 НАЙПОПУЛЯРНІШИХ АКЦІЙ ДЛЯ ДЕМОНСТРАЦІЇ (Різні сектори)
    top_tickers = [
    ...
    print(f"🚀 Запускаю масове тренування ШІ для {len(top_tickers)} компаній...")
    
    for ticker in top_tickers:
        print(f"\n{'='*50}")
        print(f"📊 СТВОРЕННЯ НЕЙРОМЕРЕЖІ ДЛЯ: {ticker}")
        print(f"{'='*50}")
        
        try:
            # Завантажуємо історію за 5 років
            df = yf.Ticker(ticker).history(period="5y")
            
            if df.empty:
                print(f"⚠️ Немає даних для {ticker}, пропускаємо...")
                continue
                
            # Готуємо дані
            X_train, y_train, scaler, raw_data = prepare_data_for_lstm(df)
            
            # Тренуємо модель (10 епох)
            train_and_save_model(X_train, y_train, ticker=ticker, epochs=10)
            
        except Exception as e:
            print(f"❌ Помилка під час тренування {ticker}: {e}")

    print("\n🎉 ФАБРИКА ШІ ЗАВЕРШИЛА РОБОТУ! Всі 20 моделей збережено у папці models/")