...
class SimulationService:
    ...
    def _build_chart_data(
        ...
        for i in range(1, horizon + 1):
            ...
            # Use a single random path (the first one) to show realistic volatility instead of a smooth average
            forecast_val = float(s_p[0][i])
            point = {"name": date_str, "forecast": round(forecast_val, 2)}
